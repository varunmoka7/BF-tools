...
class DataService {
  private pilotCompanies: PilotCompany[] = [];
  private companySummaries: CompanySummary[] = [];

  constructor() {
    this.loadPilotData();
  }

  private async loadPilotData(): Promise<void> {
    const csvPath = path.join(process.cwd(), 'data/pilot-program/pilot-companies.csv');
  ...
  private generateEnhancedCompanyData(): void {
  ...
  private getEmployeesForCompany(name: string): number {
    const companyEmployees: { [key: string]: number } = {
      'Siemens AG': 293000,
  ...
  private generateWasteDataForSector(sector: string): { totalWaste: number; recoveryRate: number; opportunityScore: number } {
    const sectorData: { [key: string]: { baseWaste: number; baseRecovery: number } } = {
  ...
  private getComplianceRisk(complexityLevel: string): string {
  ...
  public getSectorData(): SectorData[] {
    const sectorMap = new Map<string, CompanySummary[]>();
  ...
  public getCompanies(filters?: any): CompanySummary[] {
  ...
  public getCompanyById(id: string): CompanySummary | null {
    return this.companySummaries.find(c => c.companyId === id) || null;
  }

  public getUniqueCountries(): string[] {
...
├─────────────────────────────────────────┤
│  Environment: ${process.env.NODE_ENV || 'development'}              │
│  Frontend:    ${fs.existsSync(frontendDistPath) ? 'Available' : 'Not Built'}           │
│  API:         Available                 │
│  Data:        ${dataService ? 'Loaded' : 'Error'}                 │
└─────────────────────────────────────────┘
