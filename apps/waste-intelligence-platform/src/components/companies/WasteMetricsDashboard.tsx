...
interface WasteMetricsDashboardProps {
  ...
  streamsData: {
    ...
    years: string[];
    latestYear: string;
    yearSummaries?: Record<string, any>;
    metricsData?: any[];
  } | null;
}
...
export function WasteMetricsDashboard({ metricsData, streamsData }: WasteMetricsDashboardProps) {
  ...
  // Helper function to check data consistency
  const checkDataConsistency = (year: string) => {
    const yearMetrics = metricsData?.find(m => String(m.reporting_period) === year);
    const yearSummary = streamsData?.yearSummaries?.[year];
    
    if (!yearMetrics || !yearSummary) {
      return { consistent: false, message: 'Incomplete data available' };
    }
    
    const metricsTotal = yearMetrics.total_waste_generated || 0;
    const streamsTotal = yearSummary.totalGenerated || 0;
    const difference = Math.abs(metricsTotal - streamsTotal);
    const percentDifference = metricsTotal > 0 ? (difference / metricsTotal) * 100 : 0;
    
    if (percentDifference > 5) {
      return {
        consistent: false,
        message: `Data mismatch: Metrics show ${formatNumber(metricsTotal)} tons, Streams sum to ${formatNumber(streamsTotal)} tons (${percentDifference.toFixed(1)}% difference)`
      };
    }
    
    return { consistent: true, message: 'Data is consistent between metrics and waste streams' };
  };

  // Define helper functions first
  const renderMetricsForYear = (year: string) => {
    const yearMetrics = metricsData?.find(m => String(m.reporting_period) === year)
    const consistency = checkDataConsistency(year);
    ...
    return (
      <div className="space-y-6">
        ...
        <Alert className={consistency.consistent ? "border-green-200 bg-green-50" : "border-yellow-200 bg-yellow-50"}>
          <div className="flex items-center gap-2">
            ...
            <AlertDescription className={consistency.consistent ? "text-green-800" : "text-yellow-800"}>
              {consistency.message}
            </AlertDescription>
          </div>
        </Alert>
        ...
        {yearSummary && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg">
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-900">
                {formatNumber(yearSummary.totalGenerated)}
              </div>
              <div className="text-xs text-gray-600">Total Generated</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-semibold text-green-600">
                {formatNumber(yearSummary.totalRecovered)}
              </div>
              <div className="text-xs text-gray-600">Total Recovered</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-semibold text-red-600">
                {formatNumber(yearSummary.totalDisposed)}
              </div>
              <div className="text-xs text-gray-600">Total Disposed</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-semibold text-blue-600">
                {yearSummary.recoveryRate.toFixed(1)}%
              </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap w-1/6">
                    <div className="flex flex-col">
                      <span className="text-green-600 font-medium">
                        {stream.recovered !== null ? formatNumber(stream.recovered) : 'N/A'}
                      </span>
                      <span className="text-xs text-gray-500">{stream.unit}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap w-1/6">
                    <div className="flex flex-col">
                      <span className="text-red-600 font-medium">
                        {stream.disposed !== null ? formatNumber(stream.disposed) : 'N/A'}
                      </span>
                      <span className="text-xs text-gray-500">{stream.unit}</span>
                    </div>
                  </td>
